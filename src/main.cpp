...
unsigned int nTargetSpacing = 1 * 60; 						// block generation - 60 seconds 
unsigned int nRetarget = 12;							// difficulty retarget every 12 blocks
unsigned int nStakeMinAge = 24 * 60 * 60;					// min stake age - 24 hour
unsigned int nStakeMaxAge = 30 * 24 * 60 * 60;					// max stake age - 30 days
unsigned int nModifierInterval = 10 * 60;					// time to elapse before new modifier is computed
static const int64_t nTargetTimespan_legacy = nTargetSpacing * nRetarget; 	// every 720 seconds
...
const string strMessageMagic = "GameCrypto Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
	...
	{
		nSubsidy = 10000000 * COIN; // Premine 12,5% - DevSupport, IPO, Faucets, Gifts, Grants, etc
	}
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "gamecrypto", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Tei Tenga 10/12/1993 Doom's Day";
        CTransaction txNew;
        txNew.nTime = 1403104728;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1403104728;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 20096142;
		if(fTestNet)
        {
            block.nNonce   = 29756;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("df921ef16d64f335245e6b1179e13e0682ed17522e23cdd5648b34faf7054887"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xb6, 0xf7, 0xbc, 0xbe };
